 ...
 *  RPLIDAR ROS NODE
 *
 *  Copyright (c) 2021 RoboGenius Team
 * http://www.aigrantli.com
 *  Copyright (c) 2015 - 2021 Beijing HighGenius Innovation Technology Co., Ltd
 *  http://www.high-genius.com
 *
 */
...
void publish_scan(ros::Publisher *pub,
                  ...
                  float max_distance,
                  std::string frame_id, int min_angle, int max_angle)
{
 ...
 */
    ...
    for(int i = 0;i<scan_msg.ranges.size();i++)
        {
            //  裁剪掉数据范围[(min_angle*M_PI)/180 ,  (max_angle*M_PI)/180]
            //if(((scan_msg.angle_min+i*scan_msg.angle_increment) > M_PI)&&((scan_msg.angle_min+i*scan_msg.angle_increment) < M_PI*2))
            if (((scan_msg.angle_min + i * scan_msg.angle_increment)  >  (min_angle*M_PI)/180) && ((scan_msg.angle_min + i * scan_msg.angle_increment) < 0) )
                scan_msg.ranges[i] = std::numeric_limits<float>::infinity();
            else if ( ((scan_msg.angle_min + i * scan_msg.angle_increment) > 0) && ((scan_msg.angle_min + i * scan_msg.angle_increment) < (max_angle*M_PI)/180))
                scan_msg.ranges[i] = std::numeric_limits<float>::infinity();
        } 
#endif
...
int main(int argc, char * argv[]) {
    ...
    nh_private.param<int>("min_angle", min_angle, -180);
    nh_private.param<int>("max_angle", max_angle, 180);
